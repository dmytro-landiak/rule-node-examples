 ...
 */
package org.thingsboard.rule.engine.node.twilio;
...
@RuleNode(
        ...
        nodeDescription = "Sends WhatsApp message via Twilio.",
        nodeDetails = "Will send message payload as WhatsApp message via Twilio.",
...
public class TbTwilioSmsNode implements TbNode {
    ...
    private void sendSms(TbContext ctx, TbMsg msg) throws Exception {
        ...
        for (String numberTo : numbersToList) {
            Message.creator(
                    new PhoneNumber(WHATSAPP_PREFIX + numberTo.trim()),
                    new PhoneNumber(WHATSAPP_PREFIX + numberFrom.trim()),
